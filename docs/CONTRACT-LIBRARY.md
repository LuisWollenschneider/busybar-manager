...
- Upload apps do NOT appear in the catalog (they are not repo apps). `GET /api/_manager/library` lists them separately as `uploads: [{ slug, name, description, tags, installedAt, source: "upload" }]`, newest first, derived from the installed apps + their stamps. The Library → Apps subtab lists them alongside the catalog under both **All** and **Installed** (an upload is installed by definition), sorted ahead of the repo apps, plus a third filter **My apps** that hides the catalog and shows only them; each card has a two-step Remove button posting to `library/uninstall` — without it a zip-installed app could never be removed from the UI. That filter stays reachable with no repos linked (the catalog-empty states only take over when there are no catalog apps *and* no uploads).
