...
- **Named variations per app.** Save presets of argparse flags + environment variables + a draw priority. Switch a variation and the manager restarts the app for you. Managed apps draw at priority **10** by default — deliberately low, so an app you start by hand outside the manager takes the screen without a fight. Raise it per variation when you want a managed app to win.
