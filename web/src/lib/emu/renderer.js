// Vendored from busybar-emulator (web/src/lib/renderer.js, MIT, Max Swinkels)
// for busybar-manager's mirror-panel emulator mode (docs/CONTRACT.md
// "Bar-passthrough /api/_bar/*"). Adjustments from the original:
//   - the Konva draw-tool-editor TTF FontFace preloading block was dropped
//     (that editor doesn't exist here; matrix text always comes from the
//     baked glyph atlas, never those TTFs, so loading them bought us nothing
//     but a handful of pointless network requests);
//   - every asset URL the renderer fetches (icons manifest, icon SVGs,
//     animation manifest, animation frames, uploaded-image assets) is
//     rewritten to the `/api/_bar/...` proxy prefix, since these live on the
//     bar (barHost), not on the manager's own origin;
//   - kept createRenderer's canvas-size-agnostic geometry as-is: PITCH is
//     derived from the passed-in canvas's own width, so MirrorPanel can hand
//     it its existing 720×dpr backing store unchanged.
//
...
// Everything the renderer fetches (icon manifest/SVGs, animation manifest/frames,
// uploaded image assets) lives on the bar itself; from the dashboard that's only
// reachable same-origin via this proxy prefix.
const BAR = "/api/_bar";
...
export function createRenderer(cv, ocv, getModel, getStamp) {
  ...
  function loadInto(rec, src, targetH, ver) {
    ...
    // any element type whose backing image fails to load (missing/unreachable
    // asset) just never flips `ready` — drawImageEl() below then quietly skips
    // it, so one broken asset never blocks the rest of the frame.
    img.onerror = () => {};
  ...
  let ICON_INDEX = {};
  fetch(`${BAR}/public/icons.json`).then((r) => r.json()).then((cats) => { for (const cat in cats) for (const ic of cats[cat]) { const url = `${BAR}/public/icons/` + ic.path.replace(/^draw_tool\//, ""); const base = ic.fileName.replace(/\.svg$/, ""); ICON_INDEX[cat + "/" + base] = url; ICON_INDEX[base] = url; ICON_INDEX[ic.id] = url; } }).catch(() => {});

  let ANIM = {};
  fetch(`${BAR}/api/_animations`).then((r) => r.json()).then((m) => { ANIM = m || {}; }).catch(() => {});
  const animCache = {};
  ...
  function drawFrame(elements, t, frameStamp, appName) {
    ...
    for (let idx = 0; idx < elements.length; idx++) {
      ...
      if (el.type === "image" || el.path || el.stock_path) {
        ...
        if (el.path) drawImageEl(`${BAR}/assets/` + el.path, el, op, undefined, frameStamp);
        continue;
      }
