// Vendored from busybar-emulator (web/src/lib/atlas.js, MIT, Max Swinkels) for
// busybar-manager's Library-frame emulator mirror mode. Only change: the atlas
// JSON is fetched from our own /public build output instead of the emulator's
// dev path (we ship a copy at web/public/fonts/font-atlas.json — see
// docs/CONTRACT.md "Bar-passthrough" + MirrorPanel.vue for how this is used).
//
...
export function loadAtlas() {
  ...
  if (!_loading) _loading = fetch("/fonts/font-atlas.json").then((r) => r.json()).then((a) => { _atlas = a; return a; }).catch(() => null);
  return _loading;
}
